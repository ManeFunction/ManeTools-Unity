...
namespace Mane.Unity.TextMesh
{
    ...
    public class ManeText : MonoBehaviour
    {
        ...
        public Vector2 Size
        {
            get
            {
                if (_size == Vector2.zero)
                {
                    ...
                    if (info != null)
                        _size = new Vector2(info.MaxLength, CalculateBaseLine() * (info.String.Count - 1) + CalculateLineHeight());
                }
        ...
        private float CalculateBaseLine() => (float)_font.lineHeight / _font.fontSize * _fontSize + _spacingY;

        private float CalculateLineHeight() => (float)_font.ascent / _font.fontSize * _fontSize;
        ...
        private ManeTextInfo GetWrappedText()
        {
            ...
            if (_maxHeight > 0)
                maxLines = _maxHeight / (int)CalculateBaseLine();
